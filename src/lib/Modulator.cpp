...
namespace Sculptor {
	...
	Modulator::Modulator(Type type, std::optional<std::unordered_map<ModulatorProperty::Name, ModulatorProperty>> properties) : type(type) {
		...
		for (const auto& name : info[type].propertyNames) {
			...
			this->properties.at(name).setCallback([this] { form->setDirty(); });
		}
	}
